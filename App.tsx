...
const App: React.FC = () => {
  ...
  const handleAddClient = (e: React.FormEvent) => {
    ...
    const clientToAdd: Client = {
      id,
      name: newClient.name || 'Unknown Partner',
      company: newClient.company || 'Private Entity',
      email: newClient.email || '',
      phone: newClient.phone || '',
      avatar: `https://picsum.photos/seed/${id}/100`,
      status: (newClient.status as any) || 'Active',
      lastTransport: new Date().toISOString().split('T')[0]
    };
    setClients([clientToAdd, ...clients]);
    setIsAddingClient(false);
    setNewClient({ status: 'Active', company: '', name: '', email: '', phone: '' });
  };
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setActiveTab('messages')} className="text-xs font-black text-emerald-500 hover:text-white uppercase tracking-widest">Inbox (3)</button>
          </div>
          <div className="space-y-4">
            {messages.map((msg) => (
              <div key={msg.id} className="bg-emerald-900/30 border border-emerald-800/40 p-6 rounded-[32px] hover:bg-emerald-800/40 transition-all cursor-pointer group border-l-4 border-l-[#008B8B]">
                ...
                <p className="text-sm text-emerald-600 line-clamp-2 leading-relaxed italic group-hover:text-emerald-300 transition-colors">"{msg.content}"</p>
                <div className="mt-4 flex items-center justify-between">
                  <span className="text-[10px] text-emerald-800 font-black uppercase tracking-widest">{new Date(msg.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
  ...
  const renderClients = () => (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
          ...
          <button 
            ...
            className="bg-[#008B8B] hover:bg-emerald-600 text-white px-8 py-3 rounded-2xl font-black uppercase tracking-widest text-xs transition-all shadow-xl shadow-emerald-900/40 active:scale-95"
          >
            Register Partner
          </button>
        </div>
      ...
      {/* Add Client Modal */}
      {isAddingClient && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-emerald-950/95 backdrop-blur-xl animate-in fade-in duration-300">
          <div className="bg-emerald-900 border border-emerald-800 w-full max-w-2xl rounded-[48px] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 border-t-[#008B8B]/20">
            <form onSubmit={handleAddClient} className="p-10 space-y-8">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 bg-[#008B8B]/10 text-[#008B8B] rounded-[22px] flex items-center justify-center border border-[#008B8B]/20">
                    <Icons.Users className="w-7 h-7" />
                  </div>
                  <div>
                    <h3 className="text-2xl font-black text-white uppercase tracking-tighter">Register New Partner</h3>
                    <p className="text-emerald-600 text-xs font-bold uppercase tracking-widest">Global Portfolio Uplink</p>
                  </div>
                </div>
                <button type="button" onClick={() => setIsAddingClient(false)} className="p-3 bg-emerald-950/50 text-emerald-700 hover:text-white rounded-2xl transition-all">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Contact Name</label>
                  <input 
                    required
                    type="text"
                    value={newClient.name}
                    onChange={(e) => setNewClient({...newClient, name: e.target.value})}
                    placeholder="e.g. Dr. Antonio Silva"
                    className="w-full bg-emerald-950/60 border border-emerald-800/60 rounded-2xl px-5 py-4 text-white focus:ring-2 focus:ring-[#008B8B]/50 outline-none transition-all placeholder-emerald-900 font-bold"
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Organization / Company</label>
                  <input 
                    required
                    type="text"
                    value={newClient.company}
                    onChange={(e) => setNewClient({...newClient, company: e.target.value})}
                    placeholder="e.g. PharmaMed Group"
                    className="w-full bg-emerald-950/60 border border-emerald-800/60 rounded-2xl px-5 py-4 text-white focus:ring-2 focus:ring-[#008B8B]/50 outline-none transition-all placeholder-emerald-900 font-bold"
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Encrypted Email</label>
                  <input 
                    required
                    type="email"
                    value={newClient.email}
                    onChange={(e) => setNewClient({...newClient, email: e.target.value})}
                    placeholder="contact@company.it"
                    className="w-full bg-emerald-950/60 border border-emerald-800/60 rounded-2xl px-5 py-4 text-white focus:ring-2 focus:ring-[#008B8B]/50 outline-none transition-all placeholder-emerald-900 font-bold"
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Phone Link</label>
                  <input 
                    required
                    type="tel"
                    value={newClient.phone}
                    onChange={(e) => setNewClient({...newClient, phone: e.target.value})}
                    placeholder="+39 XXX XXXXXX"
                    className="w-full bg-emerald-950/60 border border-emerald-800/60 rounded-2xl px-5 py-4 text-white focus:ring-2 focus:ring-[#008B8B]/50 outline-none transition-all placeholder-emerald-900 font-bold"
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Initial Status</label>
                <div className="flex gap-4">
                  {['Active', 'Pending', 'Inactive'].map((status) => (
                    <button
                      key={status}
                      type="button"
                      onClick={() => setNewClient({...newClient, status: status as any})}
                      className={`flex-1 py-3 rounded-2xl font-black uppercase tracking-widest text-[10px] border transition-all ${
                        newClient.status === status 
                          ? 'bg-[#008B8B] border-[#008B8B] text-white' 
                          : 'bg-emerald-950/40 border-emerald-800/60 text-emerald-700 hover:border-emerald-500'
                      }`}
                    >
                      {status}
                    </button>
              ...
              <div className="pt-6">
                <button type="submit" className="w-full bg-[#008B8B] hover:bg-emerald-600 text-white h-16 rounded-3xl font-black uppercase tracking-[0.2em] transition-all shadow-xl shadow-emerald-900/60 active:scale-[0.98] text-sm">
                  Finalize Partner Integration
                </button>
              </div>
            </form>
          </div>
    ...
    <div className="animate-in fade-in slide-in-from-right-4 duration-500 max-w-5xl mx-auto space-y-8">
      ...
      <div className="space-y-8">
        {messages.map((msg) => {
          ...
          return (
            <div key={msg.id} className={`bg-emerald-900/30 border ${isHighPriority ? 'border-orange-500/50 ring-2 ring-orange-500/10 shadow-2xl' : 'border-emerald-800/40'} p-10 rounded-[48px] relative group hover:bg-emerald-800/40 transition-all duration-300`}>
               ...
               <div className="flex flex-col md:flex-row gap-8">
                  <div className="flex flex-col items-center gap-3">
                     <div className="relative">
                       ...
                       {isHighPriority && <div className="absolute -top-2 -right-2 bg-orange-500 rounded-2xl p-2 border-2 border-emerald-900 shadow-xl"><Icons.AlertTriangle className="w-4 h-4 text-white" /></div>}
                     </div>
                  ...
                  <div className="flex-1 space-y-6">
                     ...
                     <div className="flex items-center gap-6 pt-6 border-t border-emerald-800/30">
                        <button className={`${isHighPriority ? 'bg-orange-600 hover:bg-orange-500' : 'bg-[#008B8B] hover:bg-emerald-500'} text-white px-10 py-4 rounded-2xl text-xs font-black uppercase tracking-[0.2em] transition-all shadow-xl active:scale-95`}>Broadcast Reply</button>
                        <button className="bg-emerald-950/50 text-emerald-600 px-10 py-4 rounded-2xl text-xs font-black uppercase tracking-[0.2em] hover:bg-emerald-900 hover:text-white transition-all border border-emerald-800/30">Secure Archive</button>
                     </div>
                  </div>
               </div>
            </div>
    ...
    <div className="h-full space-y-8 animate-in zoom-in-95 duration-700">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <button onClick={handleSearchNearby} disabled={isSearchingNearby} className="p-3 bg-[#008B8B] hover:bg-emerald-500 text-white rounded-2xl shadow-xl transition-all disabled:opacity-50">
            {isSearchingNearby ? <div className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin"></div> : <Icons.Search className="w-5 h-5" />}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
        <div className="xl:col-span-3">
          <MapView currentLocation={transportState.currentLocation} stops={route} trafficSegments={transportState.trafficSegments} speed={transportState.speed} />
        </div>
        <div className="space-y-6">
          <div className="bg-[#008B8B]/10 border border-[#008B8B]/30 p-6 rounded-[32px] shadow-lg animate-in fade-in slide-in-from-right-4 duration-500">
            <h3 className="text-xs font-black text-[#008B8B] uppercase tracking-widest mb-6 flex items-center gap-2"><Icons.Truck className="w-5 h-5" /> Vehicle Telematics</h3>
            <div className="space-y-6">
              <div>
          ...
          <div className="bg-emerald-900/30 border border-emerald-800/40 p-6 rounded-[32px]">
            <h3 className="text-xs font-black text-emerald-500 uppercase tracking-widest mb-4 flex items-center gap-2"><Icons.Refresh className="w-4 h-4" /> Live Traffic Feed</h3>
            <div className="space-y-4">
               {transportState.trafficSegments?.map((seg) => (
                 <div key={seg.id} className="flex items-center justify-between p-3 bg-emerald-950/40 rounded-2xl border border-emerald-800/40">
                    ...
                    <span className={`text-[8px] px-2 py-0.5 rounded-full font-black uppercase tracking-widest ${seg.intensity === 'Heavy' ? 'bg-rose-500/20 text-rose-400' : seg.intensity === 'Moderate' ? 'bg-amber-500/20 text-amber-400' : 'bg-emerald-500/20 text-emerald-400'}`}>{seg.intensity}</span>
                 </div>
               ))}
            </div>
          </div>
